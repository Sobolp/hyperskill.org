 ...
 * <p>
 * The multifunctionalMapper function that accepts a list of operators (mappers) and returns a new operator.
 * The new operator just accepts a list of integer numbers and sequentially applies each mapper to each number
 * in the list (performs multiple transformations). It produces a list of transformed values.
 * Based on multifunctionalMapper, implement the multTwoAndThenAddOneTransformation operator
 * that multiplies by two each integer number and then add one to its.
 * The operator is applied to each number in the input list.
 * Based on multifunctionalMapper, implement the squareAndThenGetNextEvenNumberTransformation operator
 * that squares each integer number and then calculates the next even number following it.
 * The operator is also applied to each number in the input list.
 * There is also an example. The identityTransformation that is based on the multifunctionalMapper function identity.
 * It doesn't change values in the input list. It repeats identity transformation three times just for example.
 * <p>
 * Let's take a look at two example of how the operators should work.
 * <p>
...
class MultifunctionalMapper {
    ...
    public static final Function<List<IntUnaryOperator>, UnaryOperator<List<Integer>>> MULTIFUNCTIONAL_MAPPER =
            listOper -> listVal -> listVal.stream()
                    .map(val -> listOper.stream()
